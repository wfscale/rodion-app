...
2. Открой **SQL Editor → New query**, вставь целиком содержимое
   ...
   [`v7`](supabase/migration-v7.sql). Все идемпотентные.

   > **v5, v6 и v7 обязательны для текущей версии приложения.** v5 заводит
   > таблицу напоминаний, убирает статус «Отказ» (старые строки переезжают в
   > «Ответил — отказ») и расширяет шкалу уровней с 9 до 20. v6 добавляет
   > колонку под переписку с экспертом. v7 — щит и привал: страховку серии
   > закрытых дней. Без них приложение работает, но «Напоминания»,
   > «Переписка» и «Щит дня» попросят прогнать миграцию.
...
```bash
npm run verify   # 501 проверка логики: уровни, квота, щит, каскад, фильтры, i18n
