...
2. Открой **SQL Editor → New query**, вставь целиком содержимое
   ...
   [`v5`](supabase/migration-v5.sql). Все идемпотентные.

   > **v5 обязательна для текущей версии приложения:** она заводит таблицу
   > напоминаний, убирает статус «Отказ» (старые строки переезжают в
   > «Ответил — отказ») и расширяет шкалу уровней с 9 до 20. Без неё
   > приложение работает, но вкладка «Напоминания» покажет просьбу её прогнать.
...
```bash
npm run verify   # 352 проверки логики: уровни, квота, каскад, фильтры, i18n
